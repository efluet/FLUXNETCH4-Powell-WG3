...
month.list <- c(1:12) #c(7:8) #if set to 0, use all months; otherwise, define months (as integers from 1-12) to be processed
...
for (site.act in sites) {
  ...
  #reduce dataset to user-defined years
  year.flux=(as.integer(format(flux.time, format="%Y"))) 
  if (sel.years[1] != 0) {
  ...
  #reduce dataset to user-defined months
  month.flux=(as.integer(format(flux.time, format="%m"))) 
  if (month.list[1] != 0) {
